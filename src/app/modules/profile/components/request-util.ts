...
export const populateAcademics = (data?: any, userProfileData?: any) => {
  if (_.has(data, 'academics') && _.get(data, 'academics.length', 0) > 0) {
    const academics: any = []
...
export const getOrganisationsHistory = (form: any, userProfileData: any) => {
  ...
  const org = {
    ...
    selectBackground: _.get(form, 'selectBackground') ? form.selectBackground : userProfileData.professionalDetails[0].selectBackground,
    designation: _.get(form, 'designation') ? form.designation : userProfileData.professionalDetails[0].designation,
    professionOtherSpecify: _.get(form, 'professionOtherSpecify') ? form.professionOtherSpecify : userProfileData.professionalDetails[0].professionOtherSpecify,
    
    orgType: _.get(form, 'orgType') ? form.orgType : userProfileData.professionalDetails[0].orgType,
    orgOtherSpecify: _.get(form, 'orgOtherSpecify') ? form.orgOtherSpecify : userProfileData.professionalDetails[0].orgOtherSpecify,
    
    name: _.get(form, 'organizationName') ? _.get(form, 'organizationName')  : ( _.get(form, 'name'))? _.get(form, 'name'): '',
    ...
    locationselect: _.get(form, 'locationselect') ? form.locationselect : userProfileData.professionalDetails[0].locationselect,
    qualification: _.get(form, 'qualification') ? form.qualification : userProfileData.professionalDetails[0].qualification,
    instituteName: _.get(form, 'instituteName') ? form.instituteName : userProfileData.professionalDetails[0].instituteName,
    block: _.get(form, 'block') ? form.block : userProfileData.professionalDetails[0].block,
    subcentre: _.get(form, 'subcentre') ? form.subcentre : userProfileData.professionalDetails[0].subcentre,
    responsibilities: '',
    doj: _.get(form, 'doj') ? form.doj : userProfileData.professionalDetails[0].doj,
    description: form.orgDesc,
    completePostalAddress: '',
    additionalAttributes: {},
    // rnNumber:  _.get(form, 'rnNumber') ? form.rnNumber : userProfileData.professionalDetails[0].rnNumber,
