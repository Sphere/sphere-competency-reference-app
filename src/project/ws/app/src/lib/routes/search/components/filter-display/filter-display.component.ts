...
export class FilterDisplayComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (
      _.get( this.activated, 'parent.snapshot.data.searchPageData.data.search.tabss', searchData.search.tabss)
    ) {
      _.get( this.activated, 'parent.snapshot.data.searchPageData.data.search.tabss', searchData.search.tabss).map(
  ...
  lowerCaseFilter(filterObj: any, filterKeys: string[]) {
    filterKeys.forEach(data => {
      ...
      if (filterObj[data].value && Object.keys(filterObj[data].value).length) {
