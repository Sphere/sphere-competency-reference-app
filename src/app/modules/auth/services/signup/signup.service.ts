...
export class SignupService extends DataService {
  ...
  autoSignup(data: any): Observable<any> {
    const options = {
      url: API_END_POINTS.AUTO_USER_SIGNUP,
      data: data,
    };
  ...
  generateOtp(data: any) {
    const options = {
      url: API_END_POINTS.GENERATE_OTP,
      data: data,
    };
    return this.post(options).pipe(
      map(response => {
  ...
  autoValidateOtp(data: any) {
    const options = {
      url: API_END_POINTS.AUTO_VALIDATE_OTP,
      data: data,
    };
    return this.post(options).pipe(
      map(response => {
  ...
  setPasswordWithOtp(request: any): Observable<any> {
    ...
    //     return response
    //   }))
    return this.http.post(this.baseUrl+API_END_POINTS.VERIFY_OTP, request);
  }
