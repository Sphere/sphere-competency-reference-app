...
export class BtnPlaylistService {
  ...
  upsertPlaylist(playlistCreateRequest: NsPlaylist.IPlaylistCreateRequest, updatePlaylists = true) {
    return this.http.post<string>(`${API_PROTECTED_END_POINTS.createPlaylist}/create`, playlistCreateRequest).pipe(
      tap(_ => {
        if (updatePlaylists) {
  ...
  addToPlaylist(playlistId: string, playlistAddRequest: NsPlaylist.IPlaylistUpsertRequest, updatePlaylists = true) {
    return this.http.post<string>(`${API_PROTECTED_END_POINTS.upsertPlaylist(playlistId)}/add`, playlistAddRequest).pipe(
      tap(_ => {
        if (updatePlaylists) {
          this.updatePlaylists()
        }
  ...
  syncPlaylist(playlistId: string, updatePlaylists = true) {
    return this.http.get<NsContent.IContent[]>(`${API_PROTECTED_END_POINTS.upsertPlaylist}/sync/${playlistId}`).pipe(
      tap(_ => {
        if (updatePlaylists) {
          this.updatePlaylists()
        }
  ...
  deleteContent(playlistId: string, deleteRequest: NsPlaylist.IPlaylistUpsertRequest, updatePlaylists = true) {
    return this.http.post<string>(`${API_PROTECTED_END_POINTS.upsertPlaylist(playlistId)}/delete`, deleteRequest).pipe(
      tap(_ => {
        if (updatePlaylists) {
          this.updatePlaylists()
        }
  ...
  getAllPlaylistsApi(detailsRequired: boolean) {
    ...
    return this.http
      .get<NsPlaylist.IPlaylistResponse>(API_PROTECTED_END_POINTS.getAllPlaylists, { params })
  }
  ...
  getPlaylist(playlistId: string, type: NsPlaylist.EPlaylistTypes, sourceFields: string): Observable<NsPlaylist.IPlaylist | null> {
    ...
    return this.http
      .get<NsPlaylist.IPlaylist>(`${API_PROTECTED_END_POINTS.playlist(type)}/${playlistId}`, { params })
  }

  deletePlaylist(playlistId: string, type: NsPlaylist.EPlaylistTypes) {
    return this.http.delete(`${API_PROTECTED_END_POINTS.deletePlaylist}/${playlistId}`).pipe(
      tap(() => {
        if (this.playlistSubject[type]) {
  ...
  patchPlaylist(playlist: NsPlaylist.IPlaylist, newIDs?: string[]) {
   // tslint:disable-next-line
    ...
    return this.http.patch(`${API_PROTECTED_END_POINTS.updatePlaylists(playlist.id)}`, {
      content_ids,
  ...
  acceptPlaylist(playlistId: string) {
    return this.http.post(API_PROTECTED_END_POINTS.acceptPlaylist(playlistId), null).pipe()
  }

  rejectPlaylist(playlistId: string) {
    return this.http.post(API_PROTECTED_END_POINTS.rejectPlaylist(playlistId), null).pipe(
      tap(() => {
  ...
  sharePlaylist(shareRequest: NsPlaylist.IPlaylistShareRequest, playlistId: string) {
    return this.http.post(`${API_PROTECTED_END_POINTS.sharePlaylist}/${playlistId}`, shareRequest)
  }
  ...
  private updatePlaylists() {
    ...
    this.http
      .get<NsPlaylist.IPlaylistResponse>(API_PROTECTED_END_POINTS.getAllPlaylists).subscribe(
