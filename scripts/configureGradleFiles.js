...
function updateCapacitorPluginModuleNameSpace() {
  ...
  fs.readFile(dest, "utf8", (err, data) => {
    ...
    console.log(
      "configureGradleFiles namespace",
      codePatch,
...
function updateAppBuildGradle() {
  ...
  console.log(
    "Updated versionCode and versionName along with appId in app/build.gradle"
  );
}
...
function updateGradleProperties() {
  fs.readFile("android/gradle.properties", "utf-8", (err, data) => {
    ...
    if (!data.match("# App.properties")) {
      ...
      console.log(
        "Merged gradle properties with SUnbird properties data",
        data
      );
