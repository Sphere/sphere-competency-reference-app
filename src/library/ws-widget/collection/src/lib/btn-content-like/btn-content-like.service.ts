...
export class BtnContentLikeService {
  ...
  private fetchContentLikes() {
    ...
    this.http.get<string[]>(API_PROTECTED_END_POINTS.USER_CONTENT_LIKE).subscribe(
      likedIds => {
  ...
  like(contentId: string) {
    return this.http.post<{ [key: string]: number }>(`${API_PROTECTED_END_POINTS.USER_CONTENT_LIKE}/${contentId}`, {}).pipe(
      tap(() => {
  ...
  unlike(contentId: string) {
    return this.http.delete<{ [key: string]: number }>(`${API_PROTECTED_END_POINTS.USER_CONTENT_UNLIKE}/${contentId}`).pipe(
      tap(() => {
        this.likesReplaySubject.pipe(take(1))
          .subscribe(set => {
  ...
  fetchContentSpecificLikes(contentIds: string[]) {
    return this.http.post<{ [key: string]: number }>(`${API_PROTECTED_END_POINTS.USER_CONTENT_LIKES}`, {
